...
#add I for Input and R for RIP to new column in cdat
cdat$conditionID <- ifelse(grepl("Input", cdat$replicate), "I", "R")

#-adding the replicate number to the condition name in the cdat table
cdat$condition <- factor(cdat$condition)
...
#---Extract the results of differential RNA expression for each compartment
res <- results(dds, contrast = c("replicate", "Input", "RIP"), alpha = 0.05)

#---Extract the results of differential RNA expression for each condition
res_mito <- res[dds$condition == "Mito-APEX2", ]
res_er <- res[dds$condition == "KDEL-HRP", ]
res_nls <- res[dds$condition == "NLS-APEX2", ]
res_nes <- res[dds$condition == "NES-APEX2", ]
...
```{r echo=TRUE}
#drop NA values
res_mito_tm <- res_mito[!is.na(res_mito$padj),]
res_er_tm <- res_er[!is.na(res_er$padj),]
res_nls_tm <- res_nls[!is.na(res_nls$padj),]
res_nes_tm <- res_nes[!is.na(res_nes$padj),]

#sum and store the number of strong DE genes if not NA
strong_mito <- sum(res_mito_tm$padj < 0.05 & abs(res_mito_tm$log2FoldChange) > 1)
strong_er <- sum(res_er$padj < 0.05 & abs(res_er$log2FoldChange) > 1)
strong_nls <- sum(res_nls$padj < 0.05 & abs(res_nls$log2FoldChange) > 1)
strong_nes <- sum(res_nes$padj < 0.05 & abs(res_nes$log2FoldChange) > 1)

#df for the barplot
df <- data.frame(Condition = c("Mito", "ER", "NLS", "NES"),
                 Number = c(strong_mito, strong_er, strong_nls, strong_nes))

#barplot
ggplot(df, aes(x = Condition, y = Number)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  geom_text(aes(label = Number), vjust = -0.3, size = 3.5) +
  labs(title = "Strong DE genes", x = "Condition", y = "Number of genes")
...
top_genes <- order(top_genes, decreasing = TRUE)[1:7000]
...
metadata_sc_filt$Malignant <- ifelse(malig_cells, "Malignant", "non-Malignant")
```
...
sampled_malig <- sample(metadata_sc_filt$Malignant[metadata_sc_filt$Malignant == "Malignant"], 536, replace = FALSE)
non_malignant <- metadata_sc_filt$Malignant[metadata_sc_filt$Malignant == "non-Malignant"]
