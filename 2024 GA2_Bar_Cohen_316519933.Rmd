...
dds$condition %>% levels()
resultsNames(dds)
# print dds object
summary(dds)

dds$replicate %>% length()

dds$condition %>% length()

#add the replicate to the condition
dds$condition <- factor(paste0(dds$condition, "_", dds$replicate))


a <- results(dds, 
                    contrast = c("condition", "Mito_RIP", "Mito_Input"),
                    alpha = 0.05)
summary(a)
