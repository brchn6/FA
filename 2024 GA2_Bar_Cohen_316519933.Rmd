...
```{r echo=TRUE}
#drop NA values
res_mito <- res_mito[!is.na(res_mito$padj),]
res_er <- res_er[!is.na(res_er$padj),]
res_nls <- res_nls[!is.na(res_nls$padj),]
res_nes <- res_nes[!is.na(res_nes$padj),]

#strong DE genes
strong_mito <- sum(res_mito$padj < 0.05 & abs(res_mito$log2FoldChange) > 1)
