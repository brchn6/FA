...
strong_mito <- sum(res_Mito$padj < 0.05 & abs(res_Mito$log2FoldChange) > 1, na.rm = TRUE)
strong_er <- sum(res_ER$padj < 0.05 & abs(res_ER$log2FoldChange) > 1, na.rm = TRUE)
strong_nls <- sum(res_NES$padj < 0.05 & abs(res_NLS$log2FoldChange) > 1, na.rm = TRUE)
strong_nes <- sum(res_NLS$padj < 0.05 & abs(res_NES$log2FoldChange) > 1, na.rm = TRUE)
...
ggplot(df, aes(x = Condition, y = Number)) +
  ...
  labs(title = paste("Mito:", strong_mito, "ER:", strong_er, "NLS:", strong_nls, "NES:", strong_nes), x = "Condition", y = "Number of strong DE genes")
