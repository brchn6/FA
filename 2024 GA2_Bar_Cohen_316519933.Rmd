...
colnames(ftab) <- c("files", "condition", "replicate")
#--------------------------load the RSEM files------------------------------------------
...
cdat <- data.frame(condition = ftab$condition, replicate = ftab$replicate)
#### Make a DESeqDataSet using the design data frame
ds.txi <- DESeqDataSetFromTximport(rse, cdat, ~condition+replicate)
...
# Extract condition lavels
condition_lavels <- dds$condition %>% levels()

#create a empty list to store the results
results_list <- list()

#function to extract the results
extract_results <- function(condition, dds, alpha){
  #extract the results
  res <- results(dds, contrast = c(condition, "RIP", "Input"),alpha = alpha)
  #print the summary of the results
  print(summary(results_list[[condition]]))
  print("it works thanks god")
  return(res)
}
# apply the function to each condition
res_KDEL-HRP <-  extract_results("KDEL-HRP", dds, alpha)
results(dds, contrast = c("condition", "KDEL-HRP", "RIP", "Input"), alpha = alpha)
